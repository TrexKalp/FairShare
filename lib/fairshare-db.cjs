...
async function addExpense(expense, user) {
  ...
  try {
    ...
    await client.query(
      "INSERT INTO fairshare_trip_expenses (id, trip_id, description, amount_cents, currency, paid_by, split_all) VALUES ($1, $2, $3, $4, $5, $6, $7)",
      [expenseId, tripId, description, amountCents, currency, paidBy, splitAll],
    );
...
async function updateExpense({ expenseId, tripId, description, amount, currency, paidBy, sharedBy, splitAll, splitShares, user }) {
  const normalizedExpenseId = String(expenseId ?? "").trim();
  const normalizedTripId = String(tripId ?? "").trim();
  ...
  try {
    ...
    const updateResult = await client.query(
      `
        ...
        SET description = $1, amount_cents = $2, currency = $3, paid_by = $4, split_all = $5
        WHERE id = $6 AND trip_id = $7
      `,
      [normalizedDescription, amountCents, normalizedCurrency, normalizedPaidBy, shouldSplitAll, normalizedExpenseId, normalizedTripId],
    );
