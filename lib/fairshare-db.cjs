...
async function createTripForUser({ name, user }) {
  ...
  try {
    ...
    await client.query("INSERT INTO fairshare_trips (id, name) VALUES ($1, $2)", [tripId, normalizedName]);
    await client.query(
      "INSERT INTO fairshare_trip_people (id, trip_id, name) VALUES ($1, $2, $3) ON CONFLICT (trip_id, name) DO NOTHING",
      [randomUUID(), tripId, user.name],
    );
...
async function updateExpense({ expenseId, tripId, description, amount, paidBy, sharedBy }) {
  const normalizedExpenseId = String(expenseId ?? "").trim();
  ...
  if (!normalizedDescription || !normalizedPaidBy || normalizedSharedBy.length === 0 || !Number.isFinite(amountCents) || amountCents <= 0) {
    throw new Error("Description, amount, payer, and shared participants are required.");
  }

  await initDatabase();
  await assertTrip(normalizedTripId);
  ...
  try {
    ...
    const updateResult = await client.query(
      `
        ...
        SET description = $1, amount_cents = $2, paid_by = $3
        WHERE id = $4 AND trip_id = $5
      `,
      [normalizedDescription, amountCents, normalizedPaidBy, normalizedExpenseId, normalizedTripId],
    );
  ...
  return getGroup(normalizedTripId);
}
